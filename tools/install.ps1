...
$BackupRoot = Join-Path $BackupBase ("Ver{0}_{1}" -f $PackVersion,$Stamp)

$Items = @(
    [pscustomobject]@{ Category='communication'; Name='wifisimple'; Display='SimpleWiFi' },
    [pscustomobject]@{ Category='communication'; Name='ntpclock'; Display='NTPClock' },
    [pscustomobject]@{ Category='communication'; Name='thingspeak'; Display='ThingSpeak' },
    [pscustomobject]@{ Category='communication'; Name='httpserver'; Display='HTTP Server' },
    [pscustomobject]@{ Category='sensor'; Name='environment'; Display='Environment' }
)
...
function Find-DesktopRoot {
    ...
    foreach ($base in $candidates) {
        $root = Join-Path $base 'resources\external-resources\extensions\arduino'
        if (Test-Path -LiteralPath (Join-Path $base 'resources')) { return $root }
    }
    return $null
}
function Verify-Package {
    ...
    foreach ($file in $manifest.files) {
        $p = Join-Path $PackRoot ($file.path -replace '/', '\')
        if (-not (Test-Path -LiteralPath $p)) { throw "Missing package file: $($file.path)" }
        $actual = (Get-FileHash -Algorithm SHA256 -LiteralPath $p).Hash.ToLowerInvariant()
        if ($actual -ne ([string]$file.sha256).ToLowerInvariant()) { throw "Package verification failed: $($file.path)" }
    }
...
try {
    ...
    if ($desktopRoot) { $targets += [pscustomobject]@{ Label='Desktop'; Root=$desktopRoot } }
    else { Write-Log 'WARNING: KidsBlock desktop installation was not detected.' }
    ...
    foreach ($targetInfo in $targets) {
        ...
        foreach ($item in $Items) {
            ...
            if ($targetHash -and $targetHash -eq $sourceHash) {
                ...
                if ((Get-TreeHash $target) -ne $sourceHash) { throw "Post-copy verification failed: $target" }
    ...
    Write-Host 'Installed extensions: SimpleWiFi, NTPClock, ThingSpeak, HTTP Server, Environment'
